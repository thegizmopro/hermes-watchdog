...
function New-CircleIcon {
    ...
    switch ($Color) {
        ...
        "Red"    { $brush = [System.Drawing.Brushes]::Crimson     }
        "Yellow" { $brush = [System.Drawing.Brushes]::Gold        }
    }

    # Draw filled circle with thin dark border
    $g.FillEllipse($brush, 1, 1, 14, 14)
    $g.DrawEllipse([System.Drawing.Pens]::DarkGray, 1, 1, 14, 14)
...
$script:IconGreen  = New-CircleIcon -Color Green
...
function Test-GatewayRunning {
    ...
    # Fallback: check if hermes gateway status reports running
    try {
        $result = & $HermesExe gateway status 2>$null
...
function Get-StatusTooltip {
    ...
    if ($Status -eq "UP") {
        if ($script:UpSince) {
            $elapsed = (Get-Date) - $script:UpSince
            $hours   = [math]::Floor($elapsed.TotalHours)
            $mins    = $elapsed.Minutes
            return "Hermes: UP (${hours}h ${mins}m)"
        }
        return "Hermes: UP"
    }
    return "Hermes: DOWN"
...
function Show-LiveLog {
    ...
    $script:LogForm.Text          = "Hermes Gateway Log"
    $script:LogForm.Size          = New-Object System.Drawing.Size(800, 500)
    $script:LogForm.StartPosition = "CenterScreen"
    $script:LogForm.Icon          = $script:IconGreen
    ...
    $script:LogTextBox.ForeColor  = [System.Drawing.Color]::FromArgb(200, 220, 200)

    $script:LogForm.Controls.Add($script:LogTextBox)

    # Pre-populate with last 200 lines if log exists
    if (Test-Path $GatewayLog) {
        try {
            $tail = Get-Content $GatewayLog -Tail 200 -ErrorAction SilentlyContinue
            if ($tail) {
                $script:LogTextBox.Lines = $tail
    ...
    # Start background job to tail the log
    $script:LogTailJob = Start-Job -ScriptBlock {
        param($logPath)
        Get-Content $logPath -Wait -Tail 0 2>$null
    } -ArgumentList $GatewayLog

    # Timer to pump new log lines into the text box
    $logTimer = New-Object System.Windows.Forms.Timer
    $logTimer.Interval = 1000
    $logTimer.Add_Tick({
        if ($script:LogTailJob -and $script:LogTextBox -and !$script:LogTextBox.IsDisposed) {
            $newLines = Receive-Job $script:LogTailJob -ErrorAction SilentlyContinue
            if ($newLines) {
                $script:LogTextBox.AppendText(($newLines -join "`r`n") + "`r`n")
                # Cap at 5000 lines to avoid memory issues
                if ($script:LogTextBox.Lines.Count -gt 5000) {
                    $trimmed = $script:LogTextBox.Lines | Select-Object -Skip ($script:LogTextBox.Lines.Count - 3000)
                    $script:LogTextBox.Lines = $trimmed
                }
                $script:LogTextBox.SelectionStart = $script:LogTextBox.Text.Length
                $script:LogTextBox.ScrollToCaret()
            }
    ...
    # Cleanup on close
    $script:LogForm.Add_FormClosing({
        $logTimer.Stop()
        $logTimer.Dispose()
        if ($script:LogTailJob) {
            Stop-Job  $script:LogTailJob -ErrorAction SilentlyContinue
            Remove-Job $script:LogTailJob -Force -ErrorAction SilentlyContinue
            $script:LogTailJob = $null
        }
    })
...
function Restart-Gateway {
    ...
    # Start via scheduled task (single process chain, no duplicates)
    try {
        schtasks /Run /TN Hermes_Gateway 2>$null
    } catch {
...
function Update-Status {
    ...
    # Set yellow (checking) icon while we probe
    $notifyIcon.Icon = $script:IconYellow

    $isRunning = Test-GatewayRunning
    $newStatus = if ($isRunning) { "UP" } else { "DOWN" }

    # Update icon and tooltip
    if ($newStatus -eq "UP") {
        $notifyIcon.Icon = $script:IconGreen
        if ($script:UpSince -eq $null) {
            $script:UpSince = Get-Date
        }
    } else {
        $notifyIcon.Icon = $script:IconRed
        $script:UpSince = $null
    }

    $notifyIcon.Text = Get-StatusTooltip -Status $newStatus

    # Balloon notification on status change
    if ($script:LastStatus -ne $null -and $script:LastStatus -ne $newStatus) {
        if ($newStatus -eq "UP") {
            $notifyIcon.ShowBalloonTip(5000, "Hermes Gateway", "Gateway is UP", "Info")
        } else {
            $notifyIcon.ShowBalloonTip(5000, "Hermes Gateway", "Gateway is DOWN!", "Error")
        }
    }
...
$itemLiveLog = New-Object System.Windows.Forms.ToolStripMenuItem
$itemLiveLog.Text = "Live Log"
$itemLiveLog.Add_Click({ Show-LiveLog })

$itemRestart = New-Object System.Windows.Forms.ToolStripMenuItem
$itemRestart.Text = "Restart Gateway"
$itemRestart.Add_Click({ Restart-Gateway })

$itemTelegram = New-Object System.Windows.Forms.ToolStripMenuItem
$itemTelegram.Text = "Open Telegram"
$itemTelegram.Add_Click({ Start-Process $TelegramUrl })
